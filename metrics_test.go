...
func TestRoutesServePrometheusMetricsContract(t *testing.T) {
	...
	requireMetricFamilies(
		...
		"vpsfstatus_index_render_failures_total",
	...
	indexLastRender := requireGaugeFamily(t, families, "vpsfstatus_index_last_render_timestamp_seconds", "Unix timestamp of the last successful localized index body render.")
	requireMetricCount(t, indexLastRender, 1)
	requireUnlabeledMetricValue(t, indexLastRender, 0)
	indexLastAttempt := requireGaugeFamily(t, families, "vpsfstatus_index_last_render_attempt_timestamp_seconds", "Unix timestamp of the last localized index body render attempt.")
	requireMetricCount(t, indexLastAttempt, 1)
	requireUnlabeledMetricValue(t, indexLastAttempt, 0)
	indexRenderDuration := requireGaugeFamily(t, families, "vpsfstatus_index_render_duration_seconds", "Duration of the last successful localized index body render in seconds.")
	requireMetricCount(t, indexRenderDuration, 1)
	requireUnlabeledMetricValue(t, indexRenderDuration, 0)
	indexRenderFailures := requireCounterFamily(t, families, "vpsfstatus_index_render_failures_total", "Total number of failed localized index body render attempts.")
	requireMetricCount(t, indexRenderFailures, 1)
	requireUnlabeledCounterValue(t, indexRenderFailures, 0)
	indexRenderSkips := requireCounterFamily(t, families, "vpsfstatus_index_render_skips_total", "Total number of skipped localized index body renders because visible content did not change.")
	requireMetricCount(t, indexRenderSkips, 1)
	requireUnlabeledCounterValue(t, indexRenderSkips, 0)
